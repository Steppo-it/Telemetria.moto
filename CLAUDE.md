...
- **Calibrazione**: il bottone "Calibra" (ora nel pannello comandi) cattura gli ultimi valori grezzi (`lastRawRoll`, `lastRawPitch`, `lastRawAG`) e li usa come nuovo zero (`leanOffset`, `pitchOffset`, reset di `gEst`). I sensori restano "sempre attivi" una volta concessi i permessi — non sono legati al ciclo Avvia/Ferma, che controlla solo GPS + logging (vedi `ensureSensors()` vs `start()`/`stop()`). Questo comportamento ha avuto un effetto collaterale sul timer di sessione (ora corretto) e continua ad averne uno sulla Distanza (ancora aperto) — vedi sezione "Timer di sessione" e Problemi noti, punto 9.
...
- `sessionStartTime` viene impostato a `Date.now()` a ogni **Avvia** (`start()`) e la mini-tile "Sessione" si aggiorna una volta al secondo tramite `sessionIntervalId` (`setInterval(refreshMiniTiles, 1000)`) finché la registrazione è attiva. **Ferma** (`stop()`) ferma l'intervallo (`clearInterval`) e cattura lo scarto trascorso in `sessionFrozenMs = Date.now() - sessionStartTime` (**non azzera `sessionStartTime`**, resta disponibile per il calcolo dello snapshot). Il prossimo **Avvia** azzera sia `sessionStartTime` (nuovo `Date.now()`) sia `sessionFrozenMs` (`null`), riportando la mini-tile a 00:00.
- **Bug scoperto durante lo smoke test del Task 8, corretto in un fix successivo** (era presente nella prima versione del layout v2: non presente nella descrizione originale del piano di redesign, non corretto nel Task 8 stesso perché fuori scope — solo documentazione/verifica in quel task): poiché i sensori restano "sempre attivi" indipendentemente da Avvia/Ferma (vedi sezione Sensori sopra), ogni evento `devicemotion` richiama comunque `handleMotion()` → `renderScore()` → `refreshMiniTiles()`. Prima del fix, `miniMetricValueText('session')` ricalcolava sempre dal vivo `Date.now() - sessionStartTime`, quindi il valore in "Sessione" continuava silenziosamente ad avanzare dopo **Ferma** su un dispositivo reale (dove `devicemotion` scatta ad alta frequenza indipendentemente da Avvia/Ferma), invece di restare congelato come da spec. **Fix**: `stop()` ora cattura uno snapshot in `sessionFrozenMs`; `miniMetricValueText('session')` usa il calcolo dal vivo solo se `recording` è vero, altrimenti restituisce `formatSessionTime(sessionFrozenMs)` (o `'00:00'` se non è mai partita una sessione). Verificato con un browser headless reale: iniettando eventi `devicemotion` sintetici dopo **Ferma**, separati da un'attesa reale, il valore mostrato resta invariato finché non si preme di nuovo **Avvia**, che lo riporta correttamente a 00:00. Nota per manutenzione: la dichiarazione di `recording` è stata spostata più in alto nel file (vicino a `sessionStartTime`) per lo stesso motivo di temporal-dead-zone già documentato per le altre variabili di questa sezione — è letta dalla chiamata top-level a `refreshMiniTiles()` eseguita durante l'inizializzazione del modulo.
- **"Distanza" (`totalDistanceM`) resta un problema aperto, separato e non toccato da questo fix**: viene accumulata in `handlePosition()` a ogni fix GPS senza controllare `recording`, quindi può continuare a crescere dopo **Ferma** se il GPS resta attivo (es. navigazione in corso, `navActive`) — vedi Problemi noti, punto 9.
...
9. **Distanza (`totalDistanceM`) non si congela dopo Ferma se il GPS resta attivo** — la mini-tile Distanza continua ad avanzare dopo **Ferma** finché arrivano fix GPS (es. navigazione in corso, `navActive`), perché `handlePosition()` accumula `totalDistanceM` senza controllare `recording`. Scoperto durante lo smoke test del Task 8 (redesign layout v2) insieme al problema analogo sul timer di sessione — quest'ultimo è stato **corretto** in un fix successivo (vedi sezione "Timer di sessione" sopra: `sessionFrozenMs` congela il valore mostrato in `stop()`), ma il caso della Distanza resta aperto, non toccato dallo stesso fix (root cause diversa, in `handlePosition()`, non nella mini-tile stessa) e va indirizzato separatamente.
...
- Correggere il congelamento reale della Distanza dopo **Ferma** quando il GPS resta attivo (vedi Problemi noti, punto 9) — il timer di sessione ha già lo stesso tipo di fix applicato.
